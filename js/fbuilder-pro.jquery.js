		...
		while (flag){
            ...
            if(_match){
				var from = eq.indexOf(_match[0]),
                    ...
                    part = eq.substring(from, to);
				
                _eq += eq.substring(0, from);
                eq = eq.substr(to);
				if(_match[2] != '(' && isNaN(_match[1]*1)){
					var e = f.find('[name="'+_match[1]+'"]'), s;
					...
					if(isNaN(_match[1]*1)){ // It is a Math routine
                        _eq += part.replace(_match[0], 'Math.'+_match[0]); 
                    } else {
                        _eq += _match[0];
                    }
				}
	...
	CalcField = ( 
		function(){
			var CalcFieldClss = function(){
				// Check if the Math class contain the prec routine
			...
			CalcFieldClss.prototype = {
                ...
                defaultCalc : function(fId){
                    ...
                    if(f.length && f[0].equations && f[0].equations){
                        ...
                        for(var i=0, h = eq.length; i < h; i++){
                            var s = _calculate(f[0], eq[i].equation);
                            if( s !== false ){
                                $(eq[i].result).val(s).change();
                            }
                        }
                    }
                ...
                Calculate : function (t){
                    ...
                    var f = t.form,
						str = "[\\+|\\-|\\*|\\/|\(|\\)|%|,|:|\\?|=]+",
						reg = new RegExp(str+t.name.replace(/\[|\]/g, '')+str);
